    ...
    public class NotificacionDal : ConnectionDal
    {
		...
		public int ObtenerCantidad(UsuarioEe us)
		{
			try
			{
				...
				if (data.HasRows)
				{
					while (data.Read())
					{
						count = int.Parse(data["cantidad"].ToString() ?? string.Empty);
					}
				}
		...
		public NotificacionEe CastDto(SqlDataReader data)
		{
            var result = new NotificacionEe()
			{
				...
				FechaCreacion = DateTime.Parse(data["fecha_creacion"].ToString() ?? string.Empty),
				Leido = int.Parse(data["leido"].ToString() ?? string.Empty) == 0 ? false : true,
